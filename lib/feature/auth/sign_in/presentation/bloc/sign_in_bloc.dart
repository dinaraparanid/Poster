...
import 'package:poster/feature/auth/sign_in/presentation/bloc/sign_in_effect.dart';
import 'package:poster/feature/auth/sign_in/presentation/bloc/sign_in_event.dart';
import 'package:poster/feature/auth/sign_in/presentation/bloc/sign_in_state.dart';
  ...
  SignInBloc({required LoginUseCase loginUseCase}) : super(const SignInState()) {
    ...
    on<SignInClicked>(
      (event, emit) => loginUseCase.login(
        ...
        onSuccess: () => emit(state.copyWith(signedInUiState: const UiState.success())),
    ...
    on<SignUpClicked>(
      (event, emit) => emit(state.copyWith(
        effect: const SignInEffect.navigateToSignUp(),
      )),
    );
  }
}
